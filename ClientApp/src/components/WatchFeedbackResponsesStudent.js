...
class WatchFeedbackResponseStudent extends React.Component {
    ...
    componentDidMount() {
        userService.GetAllFeedbackStudent().then(feedbackList => this.setState({ feedbackList }));
    }

    onDownload(id, name) {
        console.log(id);
        name = name.split('.')[0];
        userService.GetFileStudentFeedback(id).then(blob => saveAs(blob, name)).catch(error => this.setState({ error }));
    }
    onRedirectToFeedback() {
        this.props.history.push({
            pathname: '/feedbackPupil',
        });
    }
                        ...
                        {this.state.feedbackList.map(feedbackEntity =>
                            <div class="card" style={{ marginTop: 1.5 + "em", marginBottom: 1 + "em", marginLeft: 1 + "em" }}>
                                ...
                                <div class="card-body">
                                    ...
                                    <br /> <Link to="/feedbackPupil" style={{ fontSize: '.9em', marginLeft: '0.5em' }}>Відкрити вкладку спілкування</Link>
                                </div>
                            </div>
                        )}
                        {this.state.error &&
...
export { WatchFeedbackResponseStudent }; 