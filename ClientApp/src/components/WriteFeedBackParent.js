...
class FeedBackParent extends React.Component {
    ...
    componentDidMount() {
        userService.GetAllChildsParent().then(children => this.setState({ children }));
    }
    ...
    onChangeRole(selected, setFieldValue, values) {
        ...
        if (selected.value === "1") {
            ...
            userService.GetAllSubjectsParent(values.zeroSelect).then(secondSelectGroup => this.setState({ secondSelectGroup })).catch(error => this.setState({ error }));
            this.setState({
                secondSelectVisibility: true,
                secondSelectDisable: false,
            ...
            userService.GetClassRoomTeacherForClassPupilParent(values.zeroSelect).then(receiverLabel => this.setState({ receiver: receiverLabel, })).catch(error => this.setState({ error }));
            this.setState({
                secondSelectVisibility: false,
            });
        }
        ...
        return (
            <Formik
                ...
                onSubmit={({ zeroSelect, firstSelect, secondSelect, thirdSelect, title, content, attachement }, { setStatus, setSubmitting }) => {
                    setStatus();
                    userService.CreateParentFeedBack(zeroSelect, firstSelect, secondSelect, thirdSelect, title, content, attachement)
                        .then(
                            user => {
                                this.props.history.push({
                                    pathname: '/watchFeedbackParent',
                                });
                            },
                            error => {
                                setSubmitting(false);
                    ...
                    <Form className="justify-content-md-center">
                        ...
                        <div className="form-group col">
                            ...
                            <Select
                                ...
                                onChange={selectChild => this.onChangeChild(selectChild, setFieldValue)} />
                        ...
                        {this.state.firstSelectVisibility === true &&
                            ...
                            <Select
                                placeholder="Оберіть тип людини..."
                                    ...
                                    classNamePrefix="select"
                                    onChange={selectRole => this.onChangeRole(selectRole, setFieldValue, values)} />
