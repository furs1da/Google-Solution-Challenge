...
class WatchStudentAnnouncements extends React.Component {
    ...
    componentDidMount() {
        userService.GetAllAnnouncementStudent().then(announcements => this.setState({ announcements }));
    }

    onDownload(id, name) {
        console.log(id);
        name = name.split('.')[0];
        userService.GetFileStudentAnnouncement(id).then(blob => saveAs(blob, name)).catch(error => this.setState({ error }));
    }
...
export { WatchStudentAnnouncements }; 